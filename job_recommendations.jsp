...
<%
    ...
    try {
        ...
        String appSql = "SELECT COUNT(*) as count FROM job_applications WHERE job_seeker_id = ?";
        PreparedStatement appStmt = conn.prepareStatement(appSql);
        appStmt.setInt(1, userId);
        ResultSet appRs = appStmt.executeQuery();
        if (appRs.next()) {
            applicationsCount = appRs.getInt("count");
        }
        appRs.close();
        appStmt.close();
        
        // Get total jobs count for recommendations
        String countSql = "SELECT COUNT(*) as count FROM jobs";
        PreparedStatement countStmt = conn.prepareStatement(countSql);
        ResultSet countRs = countStmt.executeQuery();
        if (countRs.next()) {
            recommendedJobsCount = countRs.getInt("count");
        }
        countRs.close();
        countStmt.close();
        
        // For saved jobs count, we'll use a placeholder
        savedJobsCount = 0;
...
<head>
    ...
    <title>Job Recommendations | JobPortal</title>
    ...
    <style>
        ...
        * {
        ...
        /* Welcome Section */
        .welcome-section {
            background: linear-gradient(135deg, var(--primary-very-light) 0%, #fef5f1 100%);
            border-radius: 16px;
            padding: 2rem;
            margin-bottom: 2rem;
...
<body>
    ...
    <div class="sidebar" id="sidebar">
        <div>
            ...
            <div class="menu">
                ...
                <a href="job_recommendations.jsp" class="menu-item active">
                    <i class="fas fa-briefcase"></i>
    ...
    <div class="main-content">
        ...
        <section class="welcome-section">
            <h1 class="welcome-title">Find Your Perfect Job Match</h1>
            <p class="welcome-subtitle">We've curated personalized job recommendations based on your skills, preferences, and career goals. Discover opportunities that align with your aspirations.</p>
        </section>

        <!-- Content Header -->
        <div class="content-header">
            <div>
                <h1 class="content-title">Job Recommendations</h1>
                <p class="content-subtitle">Personalized opportunities matching your profile</p>
            </div>
            <div class="content-actions">
                <button class="btn btn-secondary" id="preferencesBtn">
        ...
        <section class="jobs-section">
                ...
                <div>
                    ...
                    <% if (searchQuery != null && !searchQuery.trim().isEmpty()) { %>
                        <p style="color: var(--text-light); margin-top: 5px;">
                            <a href="job_recommendations.jsp" style="color: var(--primary-color); text-decoration: none;">
            ...
            <div class="jobs-grid">
                ...
                try {
                    Class.forName("com.mysql.cj.jdbc.Driver");
                    conn2 = DriverManager.getConnection(url, dbUser, dbPass);
                    
                    // Check if user has preferred jobs
                    String prefCheckSQL = "SELECT COUNT(*) as pref_count FROM job_preferences WHERE job_seeker_id = ? AND is_interested = 1";
                    PreparedStatement prefCheckStmt = conn2.prepareStatement(prefCheckSQL);
                    prefCheckStmt.setInt(1, userId);
                    ResultSet prefRs = prefCheckStmt.executeQuery();
                    prefRs.next();
                    int prefCount = prefRs.getInt("pref_count");
                    prefRs.close();
                    prefCheckStmt.close();

                    String sql = "";
                    if (searchQuery != null && !searchQuery.trim().isEmpty()) {
                        // SEARCH MODE: Show jobs matching search query
                        String searchPattern = "%" + searchQuery + "%";
                        sql = "SELECT j.*, c.company_name, c.description as company_description, c.industry, " +
                              ...
                              "CASE WHEN j.job_title LIKE ? THEN 1 ELSE 0 END as is_preferred " +
                              "FROM jobs j " +
                              ...
                              "WHERE j.job_title LIKE ? OR j.job_description LIKE ? OR j.requirements LIKE ? " +
                              "ORDER BY is_preferred DESC, created_at DESC";
                        
                        stmt2 = conn2.prepareStatement(sql);
                        stmt2.setString(1, searchPattern);
                        stmt2.setString(2, searchPattern);
                        stmt2.setString(3, searchPattern);
                        stmt2.setString(4, searchPattern);
                        
                    } else if (prefCount > 0) {
                        // PREFERRED MODE: Show preferred jobs first, then other jobs
                        sql = "SELECT j.*, c.company_name, c.description as company_description, c.industry, " +
                              "c.company_size, c.founded_year, c.website, c.linkedin_url, c.twitter_url, " +
                              "c.facebook_url, c.instagram_url, c.company_email, c.mission, " +
                              "ca.address_line_1, ca.address_line_2, ca.city, ca.state, ca.country, ca.postal_code, " +
                              ...
                              "LEFT JOIN companies c ON j.recruiter_id = c.user_id " +
                              "LEFT JOIN company_addresses ca ON c.id = ca.company_id AND ca.is_primary = 1 " +
                              "INNER JOIN job_preferences jp ON j.id = jp.job_id " +
                              "WHERE jp.job_seeker_id = ? AND jp.is_interested = 1 " +
                              "UNION ALL " +
                              "SELECT j.*, c.company_name, c.description as company_description, c.industry, " +
                              "c.company_size, c.founded_year, c.website, c.linkedin_url, c.twitter_url, " +
                              "c.facebook_url, c.instagram_url, c.company_email, c.mission, " +
                              "ca.address_line_1, ca.address_line_2, ca.city, ca.state, ca.country, ca.postal_code, " +
                              "0 as is_preferred " +
                              ...
                              "LEFT JOIN companies c ON j.recruiter_id = c.user_id " +
                              "LEFT JOIN company_addresses ca ON c.id = ca.company_id AND ca.is_primary = 1 " +
                              "WHERE j.id NOT IN (SELECT job_id FROM job_preferences WHERE job_seeker_id = ? AND is_interested = 1) " +
                              "ORDER BY is_preferred DESC, created_at DESC";
                        ...
                        // ALL JOBS MODE: Show all jobs with company info
                        sql = "SELECT j.*, c.company_name, c.description as company_description, c.industry, " +
                              "c.company_size, c.founded_year, c.website, c.linkedin_url, c.twitter_url, " +
                              "c.facebook_url, c.instagram_url, c.company_email, c.mission, " +
                              "ca.address_line_1, ca.address_line_2, ca.city, ca.state, ca.country, ca.postal_code, " +
                    ...
                    while (rs2.next()) {
                        ...
                        int isPreferred = rs2.getInt("is_preferred");
                ...
                <div class="job-card">
                    <div class="job-header">
                        <h3 class="job-title">
                            <%= jobTitle %>
                            <% if (isPreferred == 1) { %>
                                <span style="background: #28a745; color: white; padding: 0.2rem 0.6rem; border-radius: 12px; font-size: 0.7rem; margin-left: 0.5rem; vertical-align: middle;">
                                    <i class="fas fa-heart"></i>  Your Preferenace
                                </span>
                            <% } %>
                        </h3>
                    ...
                    <div class="job-body">
                        <p class="job-description">
                            <%= jobDescription != null ? (jobDescription.length() > 150 ? jobDescription.substring(0, 150) + "..." : jobDescription) : "No description provided" %>
                        </p>
                        <div class="job-skills">
                            <%= skillsDisplay.equals("Not specified") ? "<span class='skill-tag'>Various Skills</span>" : skillsDisplay %>
                        </div>
                        <div class="job-actions">
                            <a href="apply_job.jsp?job_id=<%= jobId %>" class="apply-btn">
                            ...
                            <button class="save-btn save-job" data-job-id="<%= jobId %>">
                ...
                <div class="no-jobs">
                    ...
                    <a href="job_recommendations.jsp" class="btn btn-primary">Refresh Jobs</a>
                </div>
                <%
                    }
    ...
    <!-- Search Preferences Popup -->
    <div class="preferences-popup" id="preferencesPopup">
        <div class="popup-content">
            <h3><i class="fas fa-search"></i> Search Preferences</h3>
            <p>Enter job titles or keywords to filter jobs (e.g., Data Analyst, Web Developer, Java)</p>
            
            <form id="preferencesForm" method="GET" action="job_recommendations.jsp">
                <textarea 
                    name="search_query" 
                    class="preferences-textarea"
                    placeholder="Enter job titles or keywords like: Data Analyst, Web Developer, Java Programmer, Marketing Manager..."
                    ><%= request.getParameter("search_query") != null ? request.getParameter("search_query") : "" %></textarea>
    ...
    <script>
        ...
        // Search Preferences button - Show popup
        ...
        preferencesPopup.addEventListener('click', (e) => {
            if (e.target === preferencesPopup) {
                preferencesPopup.style.display = 'none';
            }
        });
        ...
        applyButtons.forEach(button => {
            if (button.getAttribute('href').includes('apply_job.jsp')) {
